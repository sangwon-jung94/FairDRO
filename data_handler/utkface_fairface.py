        ...
        if self.split == 'train' or 'group' in self.version:
            ...
            for g in range(self.num_groups):
                for l in range(self.num_classes):
                    ...
                    # self.idxs_per_group[(g,l)].extend(idxs_dict['non-annotated'][(g,l)])
            
            self.features.extend(fairface_feature)
            
            # count again
            self.num_data, self.idxs_per_group = self._data_count(self.features, self.num_groups, self.num_classes)
            # num_data, tmp_idxs_per_group = self._data_count(self.features, self.num_groups, self.num_classes)
            # for g in range(self.num_groups):
            #     for l in range(self.num_classes):
            #         tmp1 = np.array(tmp_idxs_per_group[(g,l)])
            #         tmp2 = np.array(self.idxs_per_group[(g,l)])
            #         print((tmp1==tmp2).sum(), tmp1.shape)
