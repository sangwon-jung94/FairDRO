...
class SSLDataset(GenericDataset):
    ...
    def ssl_processing(self, features, num_data, idxs_per_group, idxs_dict=None):
        ...
        if self.version == 'bs1': 
            ...
            # bs 2 is to use unlabeled data only for accuracy
            ...
            for g in range(num_groups):
                for l in range(num_classes):
                    for idx in idxs_dict['non-annotated'][(g,l)]:
                       # features[idx][0] = -1 # -1 is no supervision for group id
...
#                         features[idx][0] = random.choices(idx_pool,k=1, weights=list(total_per_group[l]))[0]                        
            ...
            for g in range(num_groups):
                for l in range(num_classes):
                    new_idxs.extend(idxs_dict['annotated'][(g,l)]) # annotated
                    for idx in idxs_dict['non-annotated'][(g,l)]: # non-annotated
                        if preds[idx] != -1:
                            new_idxs.append(idx)
            ...
            folder = 'group_clf'
            filename = 'resnet18_seed{}_epochs70_bs128_lr0.001_sv{}_groupclfval.pt'.format(self.seed, self.sv_ratio)
            path = os.path.join(self.root, folder, filename)
            preds = torch.load(path)['pred']
            idx_pool = list(range(num_groups))            
            for g in range(num_groups):
            ...
            filename_pre = f'{model}_seed{self.seed}_epochs{epochs}_bs128_lr0.001'
            filename_post = f'_sv{self.sv_ratio}_groupclfval.pt'
            if self.name == 'celeba':
                if self.target_attr != 'Attractive':
                    filename_pre += f'_{self.target_attr}'
                if self.add_attr is not None:
            ...
            for g in range(num_groups):
                for l in range(num_classes):
                    for idx in idxs_dict['non-annotated'][(g,l)]:
                        if self.version == 'ours3':
                            if probs[idx].max() >= thres:
                                features[idx][0] = preds[idx].item()
                            else:
                                features[idx][0] = random.choices(idx_pool,k=1, weights=list(total_per_group[l]))[0]
            
        elif self.version == 'abla2':                        
            folder = 'group_clf'
            model = 'resnet18' if self.name in ['utkface', 'celeba','utkface_fairface'] else 'mlp'
            epochs = '70' if self.name in ['utkface', 'celeba','utkface_fairface'] else '50'
            if self.add_attr is None :
                filename = '{}_seed{}_epochs{}_bs128_lr0.001_sv{}_groupclfval.pt'.format(model, self.seed, epochs, self.sv_ratio)
            else:
                filename = '{}_seed{}_epochs{}_bs128_lr0.001_{}_sv{}_groupclfval.pt'.format(model, self.seed, epochs, self.add_attr, self.sv_ratio)
    ...
    def pick_idxs(self, num_data, idxs_per_group, filepath):
        ...
        for g in range(num_groups):
            for l in range(num_classes):
                ...
                idxs_annotated = [idx for idx in idxs_per_group[(g,l)] if not idx in idxs_nonannotated]
