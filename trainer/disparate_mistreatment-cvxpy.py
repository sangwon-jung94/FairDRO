...
#                        (N0)* cvx.sum( cvx.minimum(0, FNR_N1) )>= (N1)*cvx.sum( cvx.minimum(0, FNR_N0) )-self.epsilon]
        ...
        print("Problem is DCP (disciplined convex program):", problem.is_dcp()) #false
        print("Problem is DCCP (disciplined convex-concave program):", dccp.is_dccp(problem)) #true #solution이 unique ?
        ...
        # Parameter update
        weight = np.vstack(((-1/2)*w_opt[:-1], (1/2)*w_opt[:-1])) # class 0, class 1
