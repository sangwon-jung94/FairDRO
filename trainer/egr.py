...
class Trainer(trainer.GenericTrainer):
    def __init__(self, args, **kwargs):
        ...
        assert (self.fairness_criterion == 'eo' or self.fairness_criterion == 'ap' or self.fairness_criterion == 'dca')
    ...
    def train(self, train_loader, test_loader, epochs, dummy_loader=None, writer=None):
        ...
        self.M_matrix = self.get_M_matrix(self.fairness_criterion)
        ...
        if self.fairness_criterion == 'eo' or self.fairness_criterion == 'dca':
        ...
        S_Y_set, Y_set, S_set, self.P_S_Y_mat, self.P_Y, self.P_S = self.get_statistics(train_loader.dataset, bs=self.bs, n_workers=self.n_workers)
        ...
        for iter_ in range(n_iters):
            ...
            if self.data != 'jigsaw':
                ...
                mu_, acc_ = self.get_mu(train_loader.dataset, bs=self.bs, n_workers=self.n_workers, model=self.model)
...
#                     self.model_set.append(current_model)
                    mu_, acc_ = self.get_mu(train_loader.dataset, bs=self.bs, n_workers=self.n_workers, model=model)
                    self.theta += self.eta * (self.M_matrix @ mu_ - self.constraint_c)
...
#             model = best_model
    ...
    def get_M_matrix(self, fairness_criterion): # for eo
        if self.fairness_criterion == 'eo' or self.fairness_criterion == 'dca':
    ...
    def get_mu(self, dataset, bs=128, n_workers=2, model=None):
        model.eval()
    ...
    def Lagrangian_01(self, dataset, bs=128, n_workers=2, model=None, multiplier=None):
        model.eval()
        
        mu, acc = self.get_mu(dataset, bs=self.bs, n_workers=self.n_workers, model=model)
        error_rate = 1.0 - acc
    ...
    def get_statistics(self, dataset, bs=128, n_workers=2):
